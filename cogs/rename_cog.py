...
class Rename(commands.Cog):
    ...
    async def on_member_update(self, before, after):
        ...
        if enable_rename is None:
            logging.info(
                "\033[31mKey RemovePointsOnRename has not been set for rename_cog.py\033[0m | Not checking name renames")
            return
    ...
    async def rename_member(bot, member: discord.Member, user: DiscordUser, delta_score: int, reason):
        ...
        if should_rename is None:
            logging.info(
                "\033[31mKey ScoreNameChange has not been set for rename_cog.py\033[0m | Not renaming any users now")
            return
        ...
        score = f" [{user.social_credit}]"
        cleaned_nick = re.sub(r"\[-?\d+\]", "", user.current_name).strip()

        new_name = cleaned_nick[:25]+score
